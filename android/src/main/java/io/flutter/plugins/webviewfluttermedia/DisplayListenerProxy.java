...
/**
 * Works around an Android WebView bug by filtering some DisplayListener
 * invocations.
 *
 * <p>
 * Older Android WebView versions had assumed that when
 * {@link DisplayListener#onDisplayChanged}
 * is invoked, the display ID it is provided is of a valid display. However it
 * turns out that when a
 * display is removed Android may call onDisplayChanged with the ID of the
 * removed display, in this
 * case the Android WebView code tries to fetch and use the display with this ID
 * and crashes with an
 * NPE.
 *
 * <p>
 * This issue was fixed in the Android WebView code in
 * https://chromium-review.googlesource.com/517913 which is available starting
 * WebView version
 * 58.0.3029.125 however older webviews in the wild still have this issue.
 *
 * <p>
 * Since Flutter removes virtual displays whenever a platform view is resized
 * the webview crash
 * is more likely to happen than other apps. And users were reporting this issue
 * see:
 * https://github.com/flutter/flutter/issues/30420
 *
 * <p>
 * This class works around the webview bug by unregistering the WebView's
 * DisplayListener, and
 * instead registering its own DisplayListener which delegates the callbacks to
 * the WebView's
 ...
 * so I'm using
 * reflection to fetch all registered listeners before and after initializing a
 * webview. In the
 * first initialization of a webview within the process the difference between
 * the lists is the
...
class DisplayListenerProxy {
  ...
  void onPostWebViewInitialization(final DisplayManager displayManager) {
    ...
    // We recorded the list of listeners prior to initializing webview, any new
    // listeners we see
    ...
    if (webViewListeners.isEmpty()) {
      // The Android WebView registers a single display listener per process (even if
      // there
      // are multiple WebView instances) so this list is expected to be non-empty only
      // the
      // first time a webview is initialized.
      // Note that in an add2app scenario if the application had instantiated a non
      // Flutter
      // WebView prior to instantiating the Flutter WebView we are not able to get a
      // reference
      // to the WebView's display listener and can't work around the bug.
      //
      // This means that webview resizes in add2app Flutter apps with a non Flutter
      // WebView
      // running on a system with a webview prior to 58.0.3029.125 may crash (the
      // Android's
    ...
    for (DisplayListener webViewListener : webViewListeners) {
      ...
      // We never explicitly unregister this listener as the webview's listener is
      // never
  ...
  private static ArrayList<DisplayListener> yoinkDisplayListeners(DisplayManager displayManager) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
      ...
      // with WebView 66.0.3359.158 and the WebView version the bug this code is
      // working around was
